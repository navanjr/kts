...
sqlcmd -H. -d %dbname% -Usa -P America#1 -i C:\client\key\kts\SqlObjects\keySQLObjectDispatcher~Procedure~2.txt

bcp %dbname%.dbo.template in "Needed\bcpTemplate.txt" -Usa -PAmerica#1 -c
bcp %dbname%.dbo.Users in "Needed\bcpUsers.txt" -Usa -PAmerica#1 -c
